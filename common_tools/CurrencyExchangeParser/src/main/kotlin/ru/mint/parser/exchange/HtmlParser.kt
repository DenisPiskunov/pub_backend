...
object HTMLParser {
    fun getElementsTextByClassAsList(html: String, className: String): List<String> {
        val document = Jsoup.parse(html)
        val elementsTextList = mutableListOf<String>()
        document.html(html).getElementsByClass(className).forEach {
            if (it != null) {
                elementsTextList.add(it.text())
            }
        }
